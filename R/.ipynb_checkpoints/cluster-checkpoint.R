...
#'
#' @param data cnv_data list object of cnv data from package::prep_cnv_dat()
...
plot_clones = function(cnv_data, 
    ...
    if (type=='malig') {
        ...
        sub_normal <- cnv_data$all[variable %in% normal_bins, ]
...
#' @param hcl_sub hierarchical clustering object of cluster assignemnt as outputted from package::plot_clones()
#' @param md data.table of metadata of each bead
...
find_cluster_markers <- function(so_clone, 
    ...
    top_markers_clone <- markers_clone[order(avg_logFC,decreasing=TRUE),
...
find_go_terms <- function(cluster_markers_obj, 
    ...
    en_clone=cluster_markers_obj$markers_clone[order(avg_logFC, 
                                                     decreasing=TRUE), 
                                               package::run_enrichr(gene,50),
                                               by="cluster"]   

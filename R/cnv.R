...
prep_cnv_dat <- function(dat_bin, 
                         ...
                         hc_function = 'ward.D2', 
                         plotDir) {                                  
    # separate data into all (malignant + normal), malignant, and normal cells
    malig_cells=unique(dat_bin[cluster_type == 'Malignant',]$variable)
    ref_cells=unique(dat_bin[cluster_type == 'Normal',]$variable)
...
#'
#' @param cnv_data list object of cnv data from package::prep_cnv_dat()
...
cnv_heatmap <- function(cnv_data, 
    ...
    ann_colors = list(md_sub.cluster_type = c(Malignant="#F8776D", Normal="#00BFC4"), chr = chrom_colors)
    
    # Plot heat map
    png(file = paste0(plotDir,"/cnv_heatmap.png"), width = 800, height = 700) # png version
